...
const offerings = [
  { id: "meta", label: "Meta Ads", caption: "Paid social", icon: MetaLogo },
  { id: "instagram", label: "Instagram", caption: "Organic & ads", icon: InstagramLogo },
  { id: "react", label: "React", caption: "Websites", icon: ReactLogo },
  { id: "whatsapp", label: "WhatsApp", caption: "Automation", icon: WhatsAppLogo },
] as const;
    ...
    <section className="bg-paper py-16 md:py-20">
      <Container>
        ...
        <div className="mt-14 border-t border-line pt-10">
          <Reveal>
            <p className="eyebrow text-fg-muted">What we offer</p>
          </Reveal>
          <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
            {offerings.map((item, i) => (
              <Reveal key={item.id} delay={i * 70}>
                <div className="flex flex-col items-center gap-3 rounded-[var(--radius-card)] border border-line bg-white px-4 py-7">
                  <span className="grid h-14 w-14 place-items-center" aria-hidden>
                    <item.icon />
                  </span>
                  <div className="text-center">
                    <p className="text-sm font-bold tracking-tight text-fg">{item.label}</p>
                    <p className="mt-0.5 text-[0.75rem] text-fg-muted">{item.caption}</p>
                  </div>
                </div>
              </Reveal>
            ))}
...
function MetaLogo() {
  return (
    <svg viewBox="0 0 36 24" className="h-9 w-auto" fill="#0668E1" aria-hidden>
...
function ReactLogo() {
  return (
    <svg viewBox="-11.5 -10.23 23 20.46" className="h-10 w-10" aria-hidden>
...
function WhatsAppLogo() {
  return (
    <svg viewBox="0 0 24 24" className="h-9 w-9" fill="#25D366" aria-hidden>
